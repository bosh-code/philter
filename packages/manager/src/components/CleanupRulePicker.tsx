...
import { CleanupRule, ItemInfo } from '@philter/common';

import { MAX_MALL_PRICE, shouldWarnOnPulverize } from '../util';

import { NumericInputLite } from './NumericInputLite';
import { CleanupActionOrUnknown, SelectCleanupAction, UNKNOWN_CLEANUP_ACTION } from './SelectCleanupAction';

import './CleanupRulePicker.css';

type CleanupRulePickerProps = Readonly<{
   ...
   */
  onChange?: (newRuleOrReducer: React.SetStateAction<CleanupRule | null>) => void;
...
export const CleanupRulePicker: FC<CleanupRulePickerProps> = ({ item, rule, onChange }) => {
  ...
  return (
    <FormGroup className='CleanupRulePicker' helperText={helperText} intent={intent}>
      ...
      <div className='CleanupRulePicker__Inputs'>
        ...
        {!rule ? null : rule.action === 'GIFT' ? (
          <>
            ...
            >
              <input
                className={classNames(Classes.INPUT, Classes.SMALL, 'CleanupRulePicker__InputText')}
          ...
          <>
            ...
            >
              <input
                className={classNames(Classes.INPUT, Classes.SMALL, 'CleanupRulePicker__InputText')}
                onChange={(e) => onChange?.({ ...rule, targetItem: e.target.value })}
            ...
            >
              <span className='CleanupRulePicker__CheckBoxText'>Only use available ingredients</span>
            </Checkbox>
          </>
        ) : rule.action === 'MALL' ? (
          <FormGroup
