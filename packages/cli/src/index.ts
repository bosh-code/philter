...
import { checkProjectUpdates, loadCleanupConfig, logger, setDefaultConfig } from '@philter/common/kol';
...
function canInteractCheck(): boolean {
  ...
  return userConfirm('You are in Ronin/Hardcore. Do you want to run Philter anyway?');
}
...
export function main(): void {
  ...
  logger.warn('Philter.js is currently in alpha. It may contain bugs which could destroy your items.');
  logger.warn("If you want to stay on the safe side, halt this script and run 'ocd-cleanup.ash' instead.");
  wait(10);
  ...
  if (meatGain < 0) {
    ...
    logger.success(`Anticipated monetary gain from inventory cleansing: ${rnum(meatGain)} meat.`);
  }
}
