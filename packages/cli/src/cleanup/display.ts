  ...
  cleanupBatchExecute({
    ...
    process: (chunk) => {
      for (const [item, amount] of chunk) {
        assert.ok(putDisplay(amount, item), `Failed to batch: putDisplay(${amount}, Item.get(\`${item}\`))`);
      }
    },
    onBatchError: (chunk) => assert.fail(`Failed to put ${chunk.size} item(s) in display case`),
    shouldReplan: false
  });
