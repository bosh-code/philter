  ...
  cleanupBatchExecute({
    ...
    process: (chunk) => {
      for (const [item, amount] of chunk) {
        assert.ok(putCloset(amount, item), `Failed to batch: putCloset(${amount}, Item.get(\`${item}\`))`);
      }
    },
    onBatchError: (chunk) => assert.fail(`Failed to put ${chunk.size} item(s) in closet`),
    shouldReplan: false
  });
